...
def convertToAudio(src='video.mp4', format='wav'):
    ...
    audioSrc = f'{OUTSRC}/{filename}.{format}'
    if isNotAudioExists(audioSrc):
        if (format == 'wav'):
            audio.write_audiofile(audioSrc, codec='pcm_s16le', bitrate='16k')
        elif (format == 'mp3'):
            audio.write_audiofile(audioSrc, bitrate='320k')
        else:
            audio.write_audiofile(audioSrc)
